...
export const getNowPlayingMovies = async () => {
    try {
        const response =
            await tmdbApi.get<TmdbMovieResponse>('/movie/now_playing');
...
export const getTopRatedMovies = async () => {
    try {
        const response =
            await tmdbApi.get<TmdbMovieResponse>('/movie/top_rated');
        return response.data.results.map((item) => ({
            ...item,
            media_type: MediaType.Movie,
        }));
...
export const getUpcomingMovies = async () => {
    try {
        const response =
            await tmdbApi.get<TmdbMovieResponse>('/movie/upcoming');
        return response.data.results.map((item) => ({
            ...item,
            media_type: MediaType.Movie,
        }));
