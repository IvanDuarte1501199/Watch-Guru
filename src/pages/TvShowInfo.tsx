...
const TvShowInfo: React.FC = () => {
  ...
  const { tvShow, loading: loadingShow, error: showError } = useTvShow(id as string);

  const { allEpisodes, loading: loadingEpisodes, error: episodesError } = useSeasonEpisodes(
    id as string,
    tvShow?.seasons || []
  );
  ...
  if (loadingShow) return <p className="text-center">Loading TV show details...</p>;
